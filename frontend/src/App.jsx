...
const PostForm = ({user,visibleForums,onPostCreated}) => {
  ...
  const submit=async e=>{e.preventDefault(); if(!user?.id){toast.error('登录已失效，请重新登录');return;} const title=tRef.current?.value?.trim(),content=cRef.current?.value?.trim(); if(!title||!content)return; setSubmitting(true);
    ...
    try{const r=await apiFetch(`/posts/`,{method:'POST',body:JSON.stringify({title,content,category:cats.join(','),forum,tags:tags||null,display_name:dn,hide_uid:hu,images:imageUrls.length?imageUrls:null,is_announcement:false})});if(!r.ok)throw new Error(await errMsg(r,'发布失败'));tRef.current.value='';cRef.current.value='';setCats(['general']);setHideUid(false);setTags('');setTagInput('');setImgs([]);onPostCreated();try{const sb=document.querySelector('.create-post-card .submit-btn');if(sb){spawnBurst(sb,'🎉',12);spawnFloatPlus(sb,'发布成功 🎉')}}catch(_){}toast.success('发布成功')}catch(e){toast.error((e&&e.name==='TypeError')?'网络异常：无法连接服务器，请确认后端已启动于 localhost:8000':(e&&e.message||'发布失败'))}finally{setSubmitting(false)}}
...
const PostDetail = ({post,user,onBack,onRefresh,myStars,onToggleStar,myLikes,onToggleLike}) => {
  const toast=useToast(); const [comments,setComments]=useState([]); const [nc,setNc]=useState(''); const [loading,setLoading]=useState(false); const [err,setErr]=useState(null); const [submitting,setSubmitting]=useState(false); const [showR,setShowR]=useState(false); const [rt,setRt]=useState({type:'post',id:0}); const [editId,setEditId]=useState(null); const [editText,setEditText]=useState(''); const [savingEdit,setSavingEdit]=useState(false); const [animIds,setAnimIds]=useState([]);
  const isAdmin=user?.role==='founder'||user?.role==='ambassador';
...
const UserProfile = ({userId, onBack, onOpenPost}) => {
...
const ProfilePage = ({user,setUser,onOpenPost}) => {
  const toast=useToast(); const [nick,setNick]=useState(user.nickname); const [pw,setPw]=useState(''); const [profileBg,setProfileBg]=useState(user.profile_bg||'none');
  ...
  useEffect(()=>{ let m=true; (async()=>{ setLoadingTab(true);
    try{ const [p,c]=await Promise.all([apiFetch(`/posts/?user_id=${user.id}&limit=60`),apiFetch('/users/me/comments')]);
      if(m){ setMyPosts(p.ok?await p.json():[]); setMyComments(c.ok?await c.json():[]) } }catch{}finally{ if(m)setLoadingTab(false) } })();
  ...
  const applyBg=(key)=>{ setProfileBg(key); save({profile_bg:key}) }
  return <div className="profile-page"><div className="glass-card profile-card" style={profileBg&&profileBg!=='none'?{background:PROFILE_BGS.find(b=>b.key===profileBg)?.css}:undefined}><label className="avatar-upload"><img src={avatarUrl(user.avatar) || fallbackAvatar(user.username)} alt="头像"/><div className="avatar-upload-overlay">📷</div><input type="file" accept="image/*" onChange={async e=>{const f=e.target.files[0];if(!f)return;const fd=new FormData();fd.append('file',f);try{const r=await apiFetch(`/users/${user.id}/avatar`,{method:'POST',body:fd});if(!r.ok)throw new Error(await errMsg(r,'头像更新失败'));const u=await r.json();u.avatar=(u.avatar||'')+'?t='+Date.now();setUser(u);localStorage.setItem('user',JSON.stringify(u));toast.success('头像已更新')}catch(err){toast.error(err.message||'头像更新失败')}}}/></label><div className="profile-info"><h3>{user.nickname}</h3><p className="profile-username">@{user.username}</p><span className="uid-badge">UID: {user.uid}</span>{user.role==='founder'&&<span className="role-badge founder">创始人</span>}{user.role==='ambassador'&&<span className="role-badge ambassador">大使</span>}<div className="profile-stats"><div className="stat-item"><span className="stat-value">{user.star_count||0}</span><span className="stat-label">Star</span></div><div className="stat-item"><span className="stat-value">{user.karma||0}</span><span className="stat-label">Karma</span></div></div><div className="karma-level" style={{marginTop:'.5rem',fontSize:'.85rem',opacity:.9}}>{['🌫️ 初来乍到','🌱 成长中的声音','🔥 活跃核心','🌟 树洞之光'][Math.min(3,Math.floor((user.karma||0)/20))]}</div></div>
  <div className="profile-bg-picker"><span className="profile-bg-label">卡片背景</span><div className="bg-swatches">{PROFILE_BGS.map(b=><button key={b.key} className={`bg-swatch ${profileBg===b.key?'active':''}`} style={{background:b.css||'rgba(255,255,255,0.18)'}} title={b.label} onClick={()=>applyBg(b.key)}/>)}</div></div>
</div><div className="glass-card settings-card"><h3>设置</h3><div className="settings-list">{!isAdmin&&<div className="setting-row"><span>匿名发布</span><div className={`toggle-switch ${isAnon?'active':''}`} onClick={()=>setIsAnon(!isAnon)}/></div>}<div className="setting-row"><span>账户名</span><input value={nick} onChange={e=>setNick(e.target.value)}/></div><div className="setting-row"><span>修改密码</span><input type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="留空不修改"/></div><button className="save-btn" onClick={()=>save()}>保存设置</button></div><div className="settings-section"><h4>其他</h4><div className="settings-list"><div className="settings-item" onClick={()=>{setUser(null);localStorage.removeItem('token');localStorage.removeItem('user');toast.info('已退出登录')}}><span>退出登录</span><span className="settings-arrow">→</span></div></div></div></div>
<div className="glass-card my-stuff-card">
  <div className="my-tabs"><button className={`my-tab ${ptab==='posts'?'active':''}`} onClick={()=>setPtab('posts')}>我的帖子（{myPosts.length}）</button><button className={`my-tab ${ptab==='comments'?'active':''}`} onClick={()=>setPtab('comments')}>我的评论（{myComments.length}）</button></div>
  {loadingTab?<Spinner/>:ptab==='posts'?(myPosts.length===0?<Empty icon="📝" title="还没有发帖" desc="去首页分享点什么吧"/>:<div className="posts-list">{myPosts.map(p=><div key={p.id} className="glass-card post-card" onClick={()=>onOpenPost(p)}><h4 className="post-title">{p.title}</h4><p className="post-preview">{p.content?.slice(0,80)}{p.content?.length>80?'...':''}</p>{p.images&&p.images.length>0&&<PostImages images={p.images}/>}<div className="post-footer"><span className="post-time">{fmtTime(p.created_at)}</span><span className="action-text">💬 {p.comment_count}</span><span className="action-text">⭐ {p.star_count}</span></div></div>)}</div>):(myComments.length===0?<Empty icon="💬" title="还没有评论" desc="去帖子下聊聊吧"/>:<div className="my-comments-list">{myComments.map(c=><div key={c.id} className="glass-card my-comment-card" onClick={()=>openCommentPost(c.post_id)}><div className="my-comment-post">{c.post_title?`「${c.post_title}」`:'（帖子已删除）'}</div><p className="my-comment-content">{c.content}</p><span className="post-time">{fmtTime(c.created_at)}}</span></div>)}</div>)}
</div>
</div>
}
  ...
  return <ToastProvider>
    ...
    : <div className="app">
      <nav className="glass-nav"><h2 className="nav-title">校园树洞</h2><div className="nav-links"><button className={curPage==='home'?'active':''} onClick={()=>setCurPage('home')}>首页</button>{isAdmin&&<button className={curPage==='admin'?'active':''} onClick={()=>setCurPage('admin')}>管理</button>}<button className={`nav-bell ${notifOpen?'active':''}`} onClick={toggleNotif}>🔔{unread>0&&<span key={unread} className="notif-badge">{unread>99?'99+':unread}</span>}</button><button className={curPage==='profile'?'active':''} onClick={()=>setCurPage('profile')}>我的</button>
        <button className="nav-theme" onClick={toggleTheme} title={isLight?'切换深色':'切换浅色'} aria-label="切换主题">{isLight?'🌙':'☀️'}</button><button className="nav-msg" onClick={()=>setCurPage('chat')} title="消息" aria-label="消息">💬</button>
