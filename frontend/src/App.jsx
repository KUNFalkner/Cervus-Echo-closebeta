...
const PostForm = ({user,visibleForums,onPostCreated}) => {
  ...
  const saveDraft=()=>{ try{
    ...
    localStorage.setItem(DRAFT_KEY, JSON.stringify({title,content,forum,cats,tags,isAnon,hideUid}))
  } catch{} };
  ...
  useEffect(()=>{ try{ const d=JSON.parse(localStorage.getItem(DRAFT_KEY)||'null'); if(d){
    ...
    if(typeof d.tags==='string')setTags(d.tags); if(typeof d.isAnon==='boolean')setIsAnon(d.isAnon); if(typeof d.hideUid==='boolean')setHideUid(d.hideUid)
  } }catch{} },[]);
  useEffect(()=>{ saveDraft() },[forum,cats,tags,isAnon,hideUid]); // 状态项变化时持久化
...
const PostDetail = ({post,user,onBack,onRefresh,myStars,onToggleStar,myLikes,onToggleLike,onEditPost,setProfileUserId,setSelectedPost}) => {
        ...
        <div className={"glass-card comment-card"+(animIds.includes(String(node.id))?" comment-enter":"")+(node._pending?" comment-pending":"")}>
          <div className="comment-header">
            ...
            <div className="comment-actions">
              ...
              <button onClick={()=>{setEditId(node.id);setEditText(node.content)}} className="action-btn edit-btn">✏️</button></>}
    ...
    return <div className="dm-page"><div className="dm-list-head">私信</div>
      {convs.length===0?<Empty icon="✉️" title="还没有私信" desc="去对方主页点「私信」开始对话"/>:
        <div className="dm-list">{convs.map(c=><div key={c.id} className="dm-conv" onClick={()=>openConv(c.id)}>
          ...
          <div className="dm-conv-info"><div className="dm-conv-top"><span className="dm-conv-name">{c.peer?.nickname||'用户'}</span>{c.unread>0&&<span className="notif-badge">{c.unread>99?'99+':c.unread}</span>}</div><span className="dm-conv-last">{c.last_message||''}</span></div>
        </div>)}</div>}
    </div>
  }
...
const ProfilePage = ({user,setUser,onOpenPost,onOpenUser}) => {
...
function App() {
  ...
  const fetchPosts = useCallback(async(mode='replace')=>{ const replace=mode!=='more'
    ...
    try{ let url=`/posts/?skip=${skip}&limit=${PAGE_SIZE}`; if(activeCat!=='all')url+=`&category=${activeCat}`; if(activeForum!=='all')url+=`&forum=${activeForum}`; if(activeSort==='hot')url+=`&sort=hot`; if(debouncedQ.trim())url+=`&search=${encodeURIComponent(debouncedQ.trim())}`; if(activeTag.trim())url+=`&tag=${encodeURIComponent(activeTag.trim())}`; if(followingFeed)url+='&following=true'; const r=await apiFetch(url); if(!r.ok)throw new Error('获取帖子失败'); const data=await r.json()
    ...
    }catch(e){ if(replace) setError(e.message) }finally{ setLoading(false); setLoadingMore(false) } },[activeCat,activeForum,activeSort,debouncedQ,activeTag,user,followingFeed])
  ...
  const notifText = (n)=> n.type==='like' ? `${n.actor_name||'有人'} 赞了你的帖子` : n.type==='mention' ? `${n.actor_name||'有人'} 在评论中提到了你` : n.type==='follow' ? `${n.actor_name||'有人'} 关注了你` : n.type==='dm' ? `${n.actor_name||'有人'} 给你发了私信` : `${n.actor_name||'有人'} 回复了你的帖子`
  ...
  return <ToastProvider>
    ...
    : selectedPost ? <div className="app"><PostDetail post={selectedPost} user={user} onBack={()=>setSelectedPost(null)} onRefresh={fetchPosts} myStars={myStars} onToggleStar={toggleStar} myLikes={myLikes} onToggleLike={toggleLike} onEditPost={setEditPost} setProfileUserId={setProfileUserId} setSelectedPost={setSelectedPost}/></div>
