...
const ProfilePage = ({user,setUser,onOpenPost}) => {
  ...
  <div className="profile-bg-picker"><span className="profile-bg-label">卡片背景</span><div className="bg-swatches">{PROFILE_BGS.map(b=><button key={b.key} className={`bg-swatch ${profileBg===b.css?'active':''}`} style={{background:b.css||'rgba(255,255,255,0.18)'}} title={b.label} onClick={()=>applyBg(b.css)}/>)}<label className="bg-swatch bg-custom" title="自定义颜色"><input type="color" value={bgColorValue} onChange={e=>applyBg(e.target.value)}/></label><label className="bg-swatch bg-upload" title="上传图片背景">🖼️<input type="file" accept="image/*" onChange={async e=>{const f=e.target.files[0];if(!f)return;const fd=new FormData();fd.append('file',f);try{const r=await apiFetch(`/users/${user.id}/background`,{method:'POST',body:fd});if(!r.ok)throw new Error(await errMsg(r,'背景上传失败'));const u=await r.json();setUser(u);localStorage.setItem('user',JSON.stringify(u));setProfileBg(u.profile_bg);toast.success('背景已更新')}catch(err){toast.error(err.message||'背景上传失败')}}}/>}</label></div></div>
