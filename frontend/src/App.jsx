...
// 【站长 2026-09-17】匿名内容一律灰色默认头像（不再按名字生成花哨头像）
const GRAY_AVATAR = 'data:image/svg+xml;utf8,' + encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="32" fill="#8a8f9c"/><circle cx="32" cy="25" r="11" fill="#d7dae0"/><path d="M12 58c2-13 10-19 20-19s18 6 20 19" fill="#d7dae0"/></svg>`
)
const Avatar = ({ src, seed, className = '', anonymous = false }) => (
  <img className={`avatar-img ${className}`}
       src={anonymous ? GRAY_AVATAR : (avatarUrl(src) || fallbackAvatar(seed))} alt="" />
)
...
// 【站长 2026-09-17】带进度回调的文件上传（XHR 才有 upload.onprogress，fetch 没有）
function uploadWithProgress(path, formData, onProgress) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest()
    xhr.open('POST', path.startsWith('http') ? path : API_BASE + path)
    const tk = getToken()
    ...
    xhr.onload = () => {
      ...
      else reject(new Error((data && data.detail) || '上传失败'))
    }
...
const PostForm = ({user,visibleForums,onPostCreated}) => {
  ...
  // 手机端发帖卡默认折叠（发帖框曾占首屏 87%，内容流被推到第二屏）；展开一次后本次会话内保持展开
  const [mobileCollapsed,setMobileCollapsed]=useState(()=>window.matchMedia('(max-width: 640px)').matches);
  ...
  // 公告开关：founder 全论坛可发，大使仅本校分论坛（与后端 can_post_announcement 同步）
  const canAnnounce = user?.role==='founder' || (user?.role==='ambassador' && forum===user.school_id);
  const [asAnnouncement,setAsAnnouncement]=useState(false);
  // 仅学生可见：founder 特权（站长 2026-09-17）
  ...
  const submit=async e=>{e.preventDefault(); if(!user?.id){toast.error('登录已失效，请重新登录');return;} const title=tRef.current?.value?.trim(),content=cRef.current?.value?.trim(); if(!title||!content)return; setSubmitting(true);
    const dn=isAdmin?user.nickname:(isAnon?'匿名用户':user.nickname);
    const hu=isAdmin?false:isAnon;
    let imageUrls=[];
    if(imgs.length){ setUploading(true); setImgPct(0); try{ const fd=new FormData(); imgs.forEach(it=>fd.append('files',it.file));
        const ur=await uploadWithProgress(`/uploads/`,fd,setImgPct); imageUrls=ur.urls||[] }
      catch(err){ setUploading(false); toast.error(err.message||'图片上传失败'); return } setUploading(false) }
    try{const r=await apiFetch(`/posts/`,{method:'POST',body:JSON.stringify({title,content,category:cats.join(','),forum,tags:tags||null,display_name:dn,hide_uid:hu,images:imageUrls.length?imageUrls:null,is_announcement:asAnnouncement&&canAnnounce,audience:(studentsOnly&&isFounderUser)?'students_only':'all'})});if(!r.ok)throw new Error(await errMsg(r,'发布失败'));tRef.current.value='';cRef.current.value='';setCats(['general']);setTags('');setTagInput('');setImgs([]);setAsAnnouncement(false);setStudentsOnly(false);onPostCreated();clearDraft();try{const sb=document.querySelector('.create-post-card .submit-btn');if(sb){spawnBurst(sb,'🎉',12);spawnFloatPlus(sb,'发布成功 🎉')}}catch(_){}toast.success(asAnnouncement&&canAnnounce?'公告已发布 📢':'发布成功')}catch(e){toast.error((e&&e.name==='TypeError')?'网络异常：无法连接服务器，请确认后端已启动于 localhost:8000':(e&&e.message||'发布失败'))}finally{setSubmitting(false)}}
  return <div className="glass-card create-post-card"><h3>发布新帖子</h3>{mobileCollapsed?<button type="button" className="cp-collapsed glass-input" onClick={()=>setMobileCollapsed(false)}>✍️ 写点什么…</button>:<><form onSubmit={submit} className="create-post-form"><input ref={tRef} type="text" placeholder="帖子标题" className="glass-input" required onChange={saveDraft}/><div className="md-editor">
...
</div><div className="forum-select"><label className="forum-label">发布到：</label><div className="forum-options">{visibleForums.map(f=><button key={f.code} type="button" className={`forum-option ${forum===f.code?'active':''}`} onClick={()=>setForum(f.code)}>{f.code==='main'?'🏠 ':'🏫 '}{f.name}</button>)}</div></div>{user?.role==='student'&&<div className="post-options"><label className="checkbox-label"><input type="checkbox" checked={isAnon} onChange={e=>setIsAnon(e.target.checked)}/><span>匿名发布（不显示昵称与 UID，身份仅站长与大使可见）</span></label></div>}{canAnnounce&&<div className="post-options announce-option"><label className="checkbox-label"><input type="checkbox" checked={asAnnouncement} onChange={e=>setAsAnnouncement(e.target.checked)}/><span>📢 发布为公告（将置顶显示在 {forum==='main'?'主论坛':'本校论坛'}最上方）</span></label>{isFounderUser&&<label className="checkbox-label students-only-label"><input type="checkbox" checked={studentsOnly} onChange={e=>setStudentsOnly(e.target.checked)}/><span>👤 仅学生可见（教师/校方/大使看不到）</span></label>}</div>}<div className="category-select">{CATEGORIES.map(c=><button key={c.id} type="button" className={`category-option ${cats.includes(c.id)?'active':''}`} onClick={()=>setCats(p=>p.includes(c.id)?p.filter(x=>x!==c.id):[...p,c.id])}>{c.icon} {c.name}</button>)}</div><div className="tag-input-row"><div className="tag-chips">{tags?tags.split(',').map(x=>x.trim()).filter(Boolean).map(t=><span key={t} className="tag-chip" onClick={()=>removeTag(t)}>#{t} ✕</span>):null}<input value={tagInput} onChange={e=>setTagInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'||e.key===','||e.key===' '){e.preventDefault();addTag(tagInput)}else if(e.key==='Backspace'&&!tagInput&&tags){const arr=tags.split(',').map(x=>x.trim()).filter(Boolean);arr.pop();setTags(arr.join(','))}}} placeholder={tags?'':'添加标签（回车确认，最多5个）'} className="glass-input tag-input"/></div></div><div className="img-upload-row"><label className="img-pick-btn"><input type="file" accept="image/*" multiple onChange={pickImgs} hidden/>📷 添加图片{imgs.length?` (${imgs.length}/9)`:''}</label>{imgs.length>0&&<div className="img-thumbs">{imgs.map((it,i)=><div key={i} className="img-thumb"><img src={it.url} alt=""/><button type="button" className="img-thumb-del" onClick={()=>removeImg(i)}>✕</button></div>)}</div>}</div><button type="submit" className="glass-button submit-btn btn-primary" disabled={submitting||uploading}>{submitting?'发布中...':uploading?`图片上传中 ${imgPct}%`:'发布'}</button></form></>}</div>
}
        ...
        <div className={"glass-card comment-card"+(animIds.includes(String(node.id))?" comment-enter":"")+(node._pending?" comment-pending":"")}>
          <div className="comment-header">
            <div className="comment-author-info">
              <Avatar src={node.author_avatar} seed={node.display_name} className="comment-author-avatar" anonymous={!!node.is_anonymous} onClick={e=>{e.stopPropagation();setProfileUserId(node.user_id);setSelectedPost(null)}}/>
...
const ChatRoom = () => {
  ...
  useEffect(()=>{
    ...
    const connect = () => {
      ...
      s.onmessage = e => { try{ const m=JSON.parse(e.data); if(!m)return;
        if(m.type==='error'){ setErrTip(m.detail||'发送失败'); return }
        if(m.type==='recall'){ setMsgs(p=>p.map(x=>x.id===m.id?{...x,recalled:true,content:''}:x)); return }
        setMsgs(p=>p.some(x=>x.id&&x.id===m.id)?p:[...p,m]) }catch{} }
      s.onclose = ev => {
        if(!aliveRef.current) return
    ...
    <div className="chat-messages" ref={listRef}>
      {msgs.length===0
        ...
        : msgs.map((m,i)=><div key={m.id ?? `local-${i}`} className={`chat-message ${m.user_id===usr.id?'own':''}`}>
            ...
            {m.recalled
              ? <span className="msg-recalled">消息已撤回</span>
              : <span className="chat-content">{m.content}</span>}
            <span className="chat-time">{fmtChatTime(m.timestamp)}</span>
            {m.user_id===usr.id&&!m.recalled&&<button type="button" className="msg-recall-tag" onClick={()=>recallChat(m)}>撤回</button>}
          </div>)}
      <div ref={endRef}/>
    </div>
...
const DirectMessages = ({user, openConvId, onOpenConvChange, onOpenUser}) => {
  ...
  const recallDm=async(m)=>{ try{ const r=await apiFetch(`/dm/conversations/${activeConv}/messages/${m.id}`,{method:'DELETE'}); if(!r.ok)throw new Error(await errMsg(r,'撤回失败')); setMessages(prev=>prev.map(x=>x.id===m.id?{...x,recalled:true,content:null}:x)) }catch(e){toast.error(e.message)} };
    ...
    return <div className="dm-page"><div className="dm-list-head">私信</div>
      {convs.length===0?<Empty icon="✉️" title="还没有私信" desc="去对方主页点「私信」开始对话"/>:
        <div className="dm-list">{convs.map(c=><div key={c.id} className="dm-conv" onClick={()=>openConv(c.id)}>
          ...
          <div className="dm-conv-info"><div className="dm-conv-top"><span className="dm-conv-name">{c.peer?.nickname||'用户'}</span>{c.unread>0&&<span className="notif-badge">{c.unread>99?'99+':c.unread}</span>}<button type="button" className="conv-del-btn" title="删除会话" onClick={e=>{ e.stopPropagation(); if(confirm('删除该会话？对方不受影响')){ apiFetch(`/dm/conversations/${c.id}`,{method:'DELETE'}).then(r=>{ if(r.ok){ setConvs(prev=>prev.filter(x=>x.id!==c.id)); if(activeConv===c.id)setActiveConv(null); toast.success('会话已删除') } else { toast.error('删除失败') } }).catch(()=>toast.error('删除失败')) } }}>×</button><button type="button" className="conv-del-btn" title="删除会话" onClick={e=>{ e.stopPropagation(); if(confirm('删除该会话？对方不受影响')){ apiFetch(`/dm/conversations/${c.id}`,{method:'DELETE'}).then(r=>{ if(r.ok){ setConvs(prev=>prev.filter(x=>x.id!==c.id)); if(activeConv===c.id)setActiveConv(null); toast.success('会话已删除') } else r.json().then(d=>toast.error(d.detail||'删除失败')).catch(()=>toast.error('删除失败')) }) } }}×</button></div><span className="dm-conv-last">{c.last_time ? fmtChatTime(c.last_time) + " · " : ""}{c.last_message||""}</span></div>
        </div>)}</div>}
    </div>
  }
  ...
  return <div className="dm-page dm-thread">
    ...
    <div className="dm-messages" ref={listRef}>
      {loadingConv?<Spinner/>:shown.length===0?<Empty icon="💬" title={q?'没有匹配的消息':'还没有消息'} desc={q?'换个关键词试试':'发送第一条消息吧'}/>:
        shown.map((m,i)=><div key={m.id??`l-${i}`} className={`dm-message ${m.sender_id===meId?'own':''}`}>{m.recalled?<span className="msg-recalled">消息已撤回</span>:<><BurnMessageContent m={m} meId={meId} onView={viewBurn}/><span className="dm-msg-meta"><span className="dm-msg-time">{fmtChatTime(m.created_at)}</span>{m.sender_id===meId&&!m.burn_mode&&<><button type="button" className="msg-recall-tag" onClick={()=>recallDm(m)}>撤回</button><span className="dm-msg-receipt">{m.read?'已读':'✓'}</span></>}</span></>}</div>)}
...
const UserProfile = ({userId, user, onBack, onOpenPost, onOpenUser, onStartDM}) => {
  ...
  const load = useCallback(async()=>{ setLoading(true); setErr(null);
    try{ const [u,p]=await Promise.all([apiFetch(`/users/${userId}`),apiFetch(`/posts/?user_id=${userId}&limit=60`)]);
      if(!u.ok)throw new Error(u.status===404?'该用户的主页未公开':'加载失败'); const uj=await u.json(); const pj=p.ok?await p.json():[];
      setInfo(uj); setPosts(pj) }catch(e){ setErr(e.message||'加载失败'); setInfo(null); setPosts([]) }finally{ setLoading(false) } },[userId,tick])
  useEffect(()=>{ load() },[load])
  return <div className="app"><button className="back-btn" onClick={onBack}>← 返回</button>
    ...
    {loading?<Spinner/>:err?<ErrorBox msg={err} onRetry={()=>setTick(t=>t+1)}/>:posts.length===0?<Empty icon="📭" title="暂无帖子" desc="这个人还没有发过帖"/>:
      <div className="posts-list">{posts.map(p=><div key={p.id} className={`glass-card post-card ${p.is_pinned?'post-pinned':''}`} onClick={()=>onOpenPost(p)}>
        {p.is_announcement&&<div className="announcement-badge">📢 公告</div>}
        {(p.is_pinned>=1)&&<div className="pin-badge">📌 {p.is_pinned===2?'论坛置顶':'主页置顶'}</div>}
...
const ProfilePage = ({user,setUser,onOpenPost,onOpenUser,setStoryOpen}) => {
  ...
  const [profPublic,setProfPublic]=useState(!!user.profile_public)
  ...
  // 头像上传即时预览（选图即显示本地 blob，上传完成后切服务器地址）
  ...
  return <div className="profile-page"><div className={`glass-card profile-card ${isImgBg?'profile-card-img-bg':''}`} style={profileBg?(/^#/.test(profileBg)?{backgroundColor:profileBg}:{backgroundImage:profileBg}):undefined}><label className="avatar-upload"><img src={avatarUploading?avatarPreview:(avatarUrl(user.avatar) || fallbackAvatar(user.username))} alt="头像" style={avatarUploading?{opacity:.55}:undefined}/><div className="avatar-upload-overlay">{avatarUploading?`${avatarPct}%`:'📷'}</div>{avatarUploading&&<div className="upload-progress-bar" style={{position:'absolute',bottom:0,left:0,right:0}}><i style={{width:avatarPct+'%'}}/></div>}<input type="file" accept="image/*" disabled={avatarUploading} onChange={async e=>{const f=e.target.files[0];if(!f)return;const url=URL.createObjectURL(f);setAvatarPreview(url);setAvatarUploading(true);setAvatarPct(0);try{const fd=new FormData();fd.append('file',f);const u=await uploadWithProgress(`/users/${user.id}/avatar`,fd,setAvatarPct);u.avatar=(u.avatar||'')+'?t='+Date.now();setUser(u);localStorage.setItem('user',JSON.stringify(u));toast.success('头像已更新')}catch(err){toast.error(err.message||'头像更新失败')}finally{setAvatarUploading(false);URL.revokeObjectURL(url)}}}/></label><div className="profile-info"><h3>{user.nickname}</h3><p className="profile-username">@{user.username}</p><span className="uid-badge">UID: {user.uid}</span>{user.role==='founder'&&<span className="role-badge founder">创始人</span>}{user.role==='ambassador'&&<span className="role-badge ambassador">大使</span>}{user.role==='teacher'&&<span className="role-badge teacher">👨‍🏫 教师{user.approved===false?'（审核中）':''}</span>}{user.role==='school_official'&&<span className="role-badge school_official">🏫 学校官方</span>}<div className="profile-stats"><div className="stat-item"><span className="stat-value">{user.star_count||0}</span><span className="stat-label">Star</span></div><div className="stat-item"><span className="stat-value">{user.karma||0}</span><span className="stat-label">Karma</span></div></div><div className="profile-follow-row"><button className="follow-count-btn" onClick={()=>setShowMyFollow('followers')}>粉丝 {myStats.followers}</button><button className="follow-count-btn" onClick={()=>setShowMyFollow('following')}>关注 {myStats.following}</button></div>{showMyFollow&&<FollowListModal type={showMyFollow} userId={user.id} onClose={()=>setShowMyFollow(null)} onOpenUser={onOpenUser}/>}<div className="karma-level" style={{marginTop:'.5rem',fontSize:'.85rem',opacity:.9}}>{['🌫️ 初来乍到','🌱 成长中的声音','🔥 活跃核心','🌟 鹿鸣之光'][Math.min(3,Math.floor((user.karma||0)/20))]}</div></div>
  <div className="profile-bg-row"><button className="bg-toggle-btn" onClick={()=>setBgOpen(!bgOpen)}>{profileBg?'更换背景':'设置背景'} {bgOpen?'▲':'▼'}</button>{bgOpen&&<div className="bg-panel glass-card"><div className="bg-panel-head"><span>卡片背景</span><button className="bg-clear" onClick={()=>{applyBg('');setBgOpen(false)}}>清除</button></div><div className="bg-swatches">{PROFILE_BGS.map(b=><button key={b.key} className={`bg-swatch ${profileBg===b.css?'active':''}`} style={{background:b.css||'rgba(255,255,255,0.18)'}} title={b.label} onClick={()=>applyBg(b.css)}/>)}<label className="bg-swatch bg-custom" title="自定义颜色"><input type="color" value={bgColorValue} onChange={e=>applyBg(e.target.value)}/></label><label className="bg-swatch bg-upload" title="上传图片">{bgUploading?"⏳":""}<input type="file" accept="image/*" onChange={async e=>{const f=e.target.files[0];if(!f)return;const url=URL.createObjectURL(f);setProfileBg(`url(${url})`);setBgUploading(true);setBgPct(0);try{const fd=new FormData();fd.append('file',f);const u=await uploadWithProgress(`/users/${user.id}/background`,fd,setBgPct);if(!u)throw new Error('背景上传失败');setUser(u);localStorage.setItem('user',JSON.stringify(u));setProfileBg(u.profile_bg);toast.success('背景已更新')}catch(err){toast.error(err.message||'背景上传失败')}finally{setBgUploading(false);URL.revokeObjectURL(url)}}}/>}</label></div></div>}</div>
</div><div className="glass-card settings-card"><h3>设置</h3><div className="settings-list">{user?.role==='student'&&<div className="setting-row"><span>匿名发布</span><div className={`toggle-switch ${isAnon?'active':''}`} onClick={()=>{const v=!isAnon;setIsAnon(v);save({is_anonymous:v})}}/></div>}<div className="setting-row"><span>公开我的主页</span><div className={`toggle-switch ${profPublic?'active':''}`} onClick={()=>{const v=!profPublic;setProfPublic(v);save({profile_public:v})}}/></div><div className="setting-row"><span>账户名</span><input value={nick} onChange={e=>setNick(e.target.value)}/></div><div className="setting-row"><span>修改密码</span><input type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="留空不修改"/></div><button className="save-btn" onClick={()=>save()}>保存设置</button></div><div className="settings-section"><h4>其他</h4><div className="settings-list"><div className="settings-item" onClick={()=>setStoryOpen(true)}><span>🌌 树洞手记</span><span className="settings-arrow">→</span></div><div className="settings-item" onClick={()=>setShowPact(true)}><span>📜 社区公约</span><span className="settings-arrow">→</span></div><div className="settings-item" onClick={()=>setShowSponsor(true)}><span>☕ 支持鹿鸣</span><span className="settings-arrow">→</span></div><div className="settings-item" onClick={()=>{setUser(null);localStorage.removeItem('token');localStorage.removeItem('user');toast.info('已退出登录')}}><span>退出登录</span><span className="settings-arrow">→</span></div><div className="settings-item danger-item" onClick={()=>setShowDelete(true)}><span>注销账号</span><span className="settings-arrow">→</span></div></div></div>{showPact&&<RulesModal onClose={()=>setShowPact(false)} title="📜 鹿鸣回音社区公约"/>}{showSponsor&&<AnimatedModal onClose={()=>setShowSponsor(false)} className="sponsor-modal">{({requestClose})=>(<><h3>☕ 支持鹿鸣</h3><p className="sponsor-lead">鹿鸣回音由站长个人维护，服务器和域名的运营费用都来自自掏腰包。</p><p className="sponsor-lead">如果你在这里聊得开心，欢迎请鹿鸣喝杯咖啡——<b>完全自愿</b>，不解锁任何东西，每个功能对所有人都一样。</p><div className="sponsor-qr-wrap"><img src="/sponsor/sponsor-qr.jpg" alt="微信收款码" className="sponsor-qr"/><p className="sponsor-fallback">微信扫码 · 金额随意 · 完全自愿</p></div><p className="sponsor-thanks">谢谢你喜欢鹿鸣 🦌</p><div className="modal-actions"><button className="glass-button btn-primary" onClick={requestClose}>关闭</button></div></>)}</AnimatedModal>}{showDelete&&<AnimatedModal onClose={()=>setShowDelete(false)} className="delete-account-modal">{({requestClose})=>(<><h3>注销账号</h3><p className="delete-warn">此操作不可恢复，将永久删除你的账号、帖子与评论。</p><input type="password" value={delPw} onChange={e=>setDelPw(e.target.value)} placeholder="请输入密码确认" className="glass-input"/><div className="modal-actions"><button className="glass-button btn-secondary" onClick={requestClose}>取消</button><button className="glass-button btn-danger" onClick={deleteAccount} disabled={delBusy}>{delBusy?'注销中...':'确认注销'}</button></div></>)}</AnimatedModal>}</div>
...
function App() {
  ...
  return <ToastProvider>
    ...
    : <div className="app">
      <nav className="glass-nav"><div className="nav-brand"><h2 className="nav-title brand-title">鹿鸣回音<span className="beta-badge" title="公开测试版">BETA</span></h2><span className="brand-subtitle-en nav-subtitle-en">Cervus Echo</span></div><div className="nav-links"><button className={curPage==='home'?'active':''} onClick={()=>setCurPage('home')}>首页</button>{(isAdmin||(user?.role==='school_official'&&user?.approved!==false))&&<button className={curPage==='admin'?'active':''} onClick={()=>setCurPage('admin')}>管理</button>}<button className={curPage==='chat'?'active':''} onClick={()=>setCurPage('chat')}>消息{dmUnread>0&&<span key={'dm'+dmUnread} className="notif-badge">{dmUnread>99?'99+':dmUnread}</span>}</button><button className={curPage==='profile'?'active':''} onClick={()=>setCurPage('profile')}>我的</button><button className={`nav-bell ${notifOpen?'active':''}`} onClick={toggleNotif}>通知{unread>0&&<span key={unread} className="notif-badge">{unread>99?'99+':unread}</span>}</button><button className="nav-search nav-icon-btn" onClick={()=>{setGsSeed('');setGsOpen(true)}} title="搜索" aria-label="搜索"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{width:'15px',height:'15px',display:'block'}} aria-hidden="true"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg></button><button className="nav-theme nav-icon-btn" onClick={toggleTheme} title="点击切换">{theme==='auto'?'自动':isLight?'白天':'夜间'}</button>
      <div className="user-info nav-right-user">{isAdmin&&<span className="role-indicator">{user.role==='founder'?'👑':'🏅'}</span>}{(user.role==='teacher'||user.role==='school_official')&&<span className="role-indicator">{user.role==='teacher'?'👨‍🏫':'🏫'}</span>}<Avatar src={user.avatar} seed={user.username} className="nav-avatar" /><span className="user-nickname">{user.nickname}</span></div></div></nav>
      ...
      <main className="main-content" key={curPage} data-page={curPage}>
        {curPage==='home'&&<div className="home-page">
          ...
          <PollsSection/>
          {loading?<SkeletonList/>:error?<ErrorBox msg={error} onRetry={fetchPosts}/>:posts.length===0?<Empty icon="📝" title="暂无帖子" desc="成为第一个发帖的人吧"/>: <><div className="posts-list">{posts.map((p,i)=><div key={p.id} data-post-id={p.id} data-post-author={p.user_id} className={`glass-card post-card ${p.is_announcement?'post-announcement':''} ${p.is_pinned===2?'post-pinned':''}`} onClick={()=>setSelectedPost(p)}>{p.is_announcement&&<div className="announcement-badge">📢 公告</div>}{p.is_pinned===2&&<div className="pin-badge">📌 论坛置顶</div>}<div className="post-card-header"><Avatar src={p.author_avatar} seed={p.display_name} className="post-author-avatar" anonymous={!!p.is_anonymous} onClick={e=>{e.stopPropagation();if(p.is_anonymous&&!p.is_own)return;setProfileUserId(p.user_id);setSelectedPost(null)}}/><span className="post-author-name-small" onClick={e=>{e.stopPropagation();if(p.is_anonymous&&!p.is_own)return;setProfileUserId(p.user_id);setSelectedPost(null)}}>{p.display_name||'匿名用户'}</span>{canSeeUid(user,p)&&<span className="uid-badge">{p.user_uid}{p.hide_uid&&' (隐藏)'}</span>}<span className="post-forum-badge">{getSchoolName(p.forum)}</span><span className="post-category-mini">{p.category?p.category.split(',').map(c=>boards.find(x=>x.key===c)?.icon||'📝').join(' '):'📝'}</span></div><h4 className="post-title">{p.title}</h4><p className="post-preview">{p.content?.slice(0,100)}{p.content?.length>100?'...':''}</p>{p.images&&p.images.length>0&&<PostImages images={p.images}/>}<div className="post-footer"><span className="post-time">{fmtTime(p.created_at)}</span><div className="post-actions">{(user?.id===p.user_id||user?.role==='founder'||(user?.role==='ambassador'&&p.user_school===user.school_id))&&<><button onClick={e=>{e.stopPropagation();setEditPost(p)}} className="action-btn edit-btn" title="编辑">✏️</button><button onClick={e=>{e.stopPropagation();const canForum=(user.role==='founder'||((user.role==='school_official'||user.role==='ambassador')&&p.user_id===user.id&&p.forum===user.school_id));const scope=user.id===p.user_id?(canForum&&user.role!=='student'?'forum':(user.role==='founder'?'forum':'profile')):'forum';apiFetch(`/posts/${p.id}/pin`,{method:'PUT',body:JSON.stringify({scope,pinned:p.is_pinned!==2})}).then(r=>r.ok?fetchPosts():r.json().then(x=>toast.error(x.detail||'操作失败'))).catch(()=>toast.error('操作失败'))}} className={`action-btn pin-btn ${p.is_pinned===2?'pinned-active':''}`} title={p.is_pinned===2?'取消论坛置顶':'论坛置顶'}>📌</button><button onClick={e=>{e.stopPropagation();if(!confirm('确定删除？'))return;apiFetch(`/posts/${p.id}`,{method:'DELETE'}).then(fetchPosts)}} className="action-btn delete-btn">🗑️</button></>}<LikeButton post={p} liked={!!myLikes[p.id]} count={p.like_count} onToggle={toggleLike}/><StarButton post={p} starred={!!myStars[p.id]} count={p.star_count} onToggle={toggleStar}/><span className="action-text">💬 {p.comment_count}</span></div></div>{p.tags&&p.tags.split(',').filter(Boolean).length>0&&<div className="post-tags">{p.tags.split(',').filter(Boolean).map(t=><button key={t} className="post-tag-chip" onClick={e=>{e.stopPropagation();setTagDetail(t.trim())}}>#{t.trim()}</button>)}</div>}}</div>)}</div>{hasMore&&!loading&&<div className="load-more-wrap"><button className="load-more-btn" onClick={()=>fetchPosts('more')} disabled={loadingMore}>{loadingMore?'加载中…':'加载更多'}</button></div>}</>}
        </div>}
