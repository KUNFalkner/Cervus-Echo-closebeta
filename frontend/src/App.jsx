...
// ── PostDetail ──
const PostDetail = ({post,user,onBack,onRefresh,myStars,onToggleStar,myLikes,onToggleLike,onEditPost}) => {
...
function App() {
  ...
  const fetchDmUnread=useCallback(async()=>{ if(!user)return; try{ const r=await apiFetch('/dm/conversations'); if(r.ok){ const cs=await r.json(); setDmUnread(cs.reduce((a,c)=>a+(c.unread||0),0)) } }catch{} },[user])
  ...
  return <ToastProvider>
    ...
    : selectedPost ? <div className="app"><PostDetail post={selectedPost} user={user} onBack={()=>setSelectedPost(null)} onRefresh={fetchPosts} myStars={myStars} onToggleStar={toggleStar} myLikes={myLikes} onToggleLike={toggleLike} onEditPost={setEditPost}/></div>
