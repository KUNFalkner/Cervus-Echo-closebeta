...
const AdminPage = ({user}) => {
  ...
  const loadTApprovals = useCallback(async()=>{ if(!isFounder)return; try{ const r=await apiFetch('/admin/teacher-approvals'); if(r.ok)setTApprovals(await r.json()) }catch{} },[isFounder])
  useEffect(()=>{ if(tab==='teachers'&&isFounder) loadTApprovals() },[tab,loadTApprovals])
  ...
  {tab==='teachers'&&isFounder&&<div className="glass-card" style={{padding:'1rem'}}>
    ...
    <p style={{fontSize:'.72rem',color:'var(--muted)',margin:'.5rem 0 0'}}>每校仅一个官方账号；创建后把用户名密码线下交给学校。</p>
  </div>}
  ...
  return <ToastProvider>
    ...
    : <div className="app">
      ...
      <main className="main-content" key={curPage} data-page={curPage}>
        ...
        {curPage==='chat'&&<div className="chat-page"><div className="chat-tabs"><button className={`chat-tab ${chatTab==='global'?'active':''}`} onClick={()=>setChatTab('global')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{width:'14px',height:'14px',verticalAlign:'-2px'}} aria-hidden="true"><path d="M21 12a8.5 8.5 0 0 1-8.5 8.5c-1.3 0-2.6-.3-3.7-.8L3.5 21l1.4-4.2A8.5 8.5 0 1 1 21 12z"/></svg> 聊天室</button><button className={`chat-tab ${chatTab==='dm'?'active':''}`} onClick={()=>setChatTab('dm')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{width:'14px',height:'14px',verticalAlign:'-2px'}} aria-hidden="true"><rect x="2.5" y="5" width="19" height="14" rx="2.5"/><path d="m3.5 6.5 8.5 6.5 8.5-6.5"/></svg> 私信{dmUnread>0&&<span className="notif-badge">{dmUnread>99?'99+':dmUnread}</span>}</button><button className={`chat-tab ${chatTab==='group'?'active':''}`} onClick={()=>setChatTab('group')}><svg viewBox="0 0 24 24" fill="currentColor" style={{width:'14px',height:'14px',verticalAlign:'-2px'}} aria-hidden="true"><circle cx="9" cy="8" r="3.4"/><path d="M2.5 19c.7-3.6 3.3-5.5 6.5-5.5s5.8 1.9 6.5 5.5z"/><circle cx="17" cy="9" r="2.6"/><path d="M15.5 13.8c2.9.2 5.2 1.9 6 5.2h-5.1c-.2-2-1-3.7-2.3-4.8.5-.3 1-.4 1.4-.4z"/></svg> 群聊</button></div>{chatTab==='global'?<div className="glass-card chat-container"><ChatRoom/></div>:chatTab==='dm'?<DirectMessages user={user} openConvId={openConvId} onOpenConvChange={setOpenConvId} onOpenUser={setProfileUserId}/>:<GroupChat user={user} onOpenUser={setProfileUserId}/>}</div>}
