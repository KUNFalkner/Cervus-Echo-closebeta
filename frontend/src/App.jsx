...
const PostDetail = ({post,user,onBack,onRefresh,myStars,onToggleStar,myLikes,onToggleLike}) => {
  const toast=useToast(); const [comments,setComments]=useState([]); const [nc,setNc]=useState(''); const [loading,setLoading]=useState(false); const [err,setErr]=useState(null); const [submitting,setSubmitting]=useState(false); const [showR,setShowR]=useState(false); const [rt,setRt]=useState({type:'post',id:0}); const [editId,setEditId]=useState(null); const [editText,setEditText]=useState(''); const [savingEdit,setSavingEdit]=useState(false);
  const isAdmin=user?.role==='founder'||user?.role==='ambassador';
