...
（分支/子目录 `tarot-guide`，MIT 协议）的一份**逐字副本**，用于给鹿鸣回音塔罗的本地大模型
...
而鹿鸣回音塔罗是**单次调用**：前端已抽好牌并把牌名/正逆位/位置发到
...
- 原 skill 输出模板带 `![牌](图片URL)` 与 Markdown 标题；鹿鸣回音前端是**纯文本**渲染解读、
  牌面图由前端自己用 `tarotData.js` 展示，故输出保持纯文本（吸收其结构但不加 Markdown 图片）。
- 鹿鸣回音前端目前只支持两种牌阵：`time`（过去·现在·未来 3 张）与 `celtic`（凯尔特十字 10 张），
