...
rem  Campus Treehole - production preview launcher
...
"%NGINX%\nginx.exe" -p "%NGINX%" -c conf\treehole\nginx.conf -s stop >nul 2>&1
...
start "treehole-backend" /D "%ROOT%\backend" /MIN cmd /c "uvicorn app.main:app --host 127.0.0.1 --port 8000"
